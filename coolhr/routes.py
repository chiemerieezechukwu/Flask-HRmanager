...
from coolhr.forms import (
    ...
    TrainingForm,
    ResetPasswordForm,
    ResetPasswordnUsernameRequestForm,
...
def index():
    if session.get("company_email"):
        ...
        company = Companies.query.join(
            Employees, Employees.employee_email == session.get("employee_email")
        ).first()
...
def company_signup():
    ...
    if form.validate_on_submit():
        company = Companies(
            company_name=form.company_username.data,
            company_username=form.company_username.data,
...
def login(company_username):
    ...
    company2 = Companies.query.filter_by(
        company_username=company_username
    ).first_or_404()
    company = Companies.query.filter_by(
        company_email=form.email.data, company_id=company2.company_id
    ).first()
    employee = Employees.query.filter_by(
        employee_email=form.email.data, company_id=company2.company_id
    ).first()
    if form.validate_on_submit():
...
def employeeregister(company_username):
    ...
    if form.validate_on_submit():
        employee = Employees(
            ...
            employee_surname=form.employee_surname.data,
    ...
    if form.validate_on_submit():
        if company:
            ...
            return redirect(
                url_for(
                    "login",
                    company_username=e_company.company_username,
                    n_email=employee.employee_email,
                )
            )
    return render_template(
        "reset_password.html",
...
def recover_company_username():
    ...
    if form.validate_on_submit():
        ...
        if company:
            ...
            company_username = (
                Companies.query.filter_by(company_id=employee.company_id)
                .first()
                .company_username
            )
...
def trainings(company_username):
    ...
    training = Trainings.query.filter_by(
        training_name=form.training_name.data, company_id=company.company_id
    ).first()
    ...
    if form.training_submit.data:
        if form.validate_on_submit():
            if training is None:
                ...
                db.session.commit()
                flash(
                    "Training has been published. Employees can now view them and subscribe"
                )
                return redirect(url_for("trainings", company_username=company_username))
            else:
                flash("There's a training with a similar name")
    return render_template(
        "create_trainings.html",
        form=form,
        training=available_trainings,
        company_username=company_username,
    )
...
def manage_trainings(company_username):
    ...
    if request.form.get("mark_complete"):
        if training.training_status is not False:
...
def trainings_deets(company_username):
    ...
    training = Trainings.query.filter_by(
        training_name=request.args.get("v"), company_id=company.company_id
    ).first_or_404()
    training2 = Trainings.query.filter_by(
        training_name=form.training_name.data, company_id=company.company_id
    ).first()
            ...
            if training is not None and training.training_status != False:
                if training.training_name != form.training_name.data:
                    if training2 is None:
                        ...
                        return redirect(
                            url_for("trainings", company_username=company_username)
                        )
                    else:
                        flash("There's a training with a similar name")
    ...
    if request.form.get("add"):
        employee = Employees.query.filter_by(
            employee_id=request.form.get("add"), company_id=company.company_id
        ).first()
            ...
            if training is not None:
                if training.training_status != False:
                    if training not in employee.training_subscriptions:
                        ...
                        db.session.commit()
                        flash(
                            "You've successfully added {} to {}".format(
                                employee.employee_name, training.training_name
                            )
                        )
                    else:
                        flash(
                            "{} is already subscribed to {}".format(
                                employee.employee_name, training.training_name
                            )
                        )
                        return redirect(
    ...
    # remove employee from training
        ...
        employee = Employees.query.filter_by(
            employee_id=request.form.get("remove"), company_id=company.company_id
        ).first()
        if employee is not None:
            if training is not None:
                if training.training_status != False:
                    if training in employee.training_subscriptions:
                        ...
                        db.session.commit()
                        flash(
                            "You've successfully removed {} from {}".format(
                                employee.employee_name, training.training_name
                            )
                        )
                    else:
                        flash(
                            "{} is already unsubscribed from {}".format(
                                employee.employee_name, training.training_name
                            )
                        )
                        return redirect(
                ...
                flash("This training has been deleted")
    return render_template(
        "training_deets.html", form=form, training=training, company=company
    )
...
def training_subscription(company_username):
    ...
    if request.form.get("subscribe"):
        trainings = Trainings.query.filter_by(
            training_id=request.form.get("subscribe"), training_status=True
        ).first()
        ...
        trainings = Trainings.query.filter_by(
            training_id=request.form.get("unsubscribe"), training_status=True
        ).first()
        if trainings is not None:
            if trainings in employee.training_subscriptions:
                ...
                flash(
                    "You're already unsubscribed from {}".format(
    ...
    if form.employee_submit.data:
        if form.validate_on_submit():
            if (
                ...
                or form.employee_surname.data != employee.employee_surname
                or form.employee_username.data != employee.employee_username
                or form.employee_email.data != employee.employee_email
            ):
