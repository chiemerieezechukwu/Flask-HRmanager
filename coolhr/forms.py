...
from coolhr.models import Employees, Companies, Trainings


def _user():
    if session.get("company_email"):
        user = Companies.query.filter_by(
            company_email=session.get("company_email")
        ).first()
    elif session.get("employee_email"):
        user = Employees.query.filter_by(
...
class CompanyRegistrationForm(FlaskForm):
    company_name = StringField(
        "Company Name", validators=[DataRequired(), Length(min=4, max=64)]
    )
    company_username = StringField(
        "Username",
        validators=[DataRequired(), Length(min=4, max=64)],
        render_kw={"placeholder": "username"},
    )
    company_email = StringField(
        "Email", validators=[DataRequired(), Email(), Length(min=4, max=64)]
    )
    company_password = PasswordField("Password", validators=[DataRequired()])
    company_password2 = PasswordField(
        "Repeat Password",
        validators=[
            DataRequired(),
...
class EmployeeRegistrationForm(FlaskForm):
    employee_name = StringField(
        "Name", validators=[DataRequired(), Length(min=4, max=64)]
    )
    employee_surname = StringField(
        "Surname", validators=[DataRequired(), Length(min=4, max=64)]
    )
    employee_username = StringField(
    ...
    employee_password2 = PasswordField(
        "Repeat Password",
        validators=[
            DataRequired(),
            EqualTo("employee_password", message="passwords must match"),
        ],
    )
...
class TrainingForm(FlaskForm):
    training_name = StringField(
        "Traning Name", validators=[DataRequired(), Length(min=4, max=64)]
    )
    training_description = TextAreaField(
        "Training Description", validators=[Length(min=0, max=1024)]
    )
    ...
    def validate_employee_username(self, username):
        ...
        if current_user.employee_username != username.data:
            ...
            employee = Employees.query.filter_by(
                employee_username=username.data
            ).first()
            if company or employee is not None:
                raise ValidationError(
                    "Username is already taken. Please choose a different one"
...
class UploadImageForm(FlaskForm):
    image = FileField(
        ...
        validators=[
            DataRequired(),
            FileAllowed(["jpg", "png", "jpeg", "gif"], "We only support JPG or PNG pictures."),
        ],
    )
